...
sta, dxSta, dySta = tools.Read_Seismic_Stations(
    file=path + "Seismic/IA_StCoordinates.txt", ori_prof=ori_prof
)
...
stream = tools.Read_Traces(
    RF_list=path + "Seismic/" + rf_list, sta=sta, ori_prof=ori_prof
)
...
# stream = tools.tracing_2D(
#     stream=stream,
#     ori_prof=ori_prof,
#     path_velocity_model=path_velocity_model,
...
mObs = tools.ccpM(
    stream, migration_params, sta, phase="PS", stack=0, dbaz=180, bazmean=180
)
